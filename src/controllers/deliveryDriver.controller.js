...
exports.listAssignments = async (req, res) => {
  try {
    const assignments = await deliveryDriverService.listActiveAssignments(req.user);
...
exports.listHistory = async (req, res) => {
  try {
    const assignments = await deliveryDriverService.listDeliveryHistory(req.user, {
...
exports.getAssignment = async (req, res) => {
  try {
    const assignment = await deliveryDriverService.getAssignmentDetail(req.user, req.params.assignmentId);
    res.json({ assignment });
  } catch (err) {
...
exports.syncOffline = async (req, res) => {
  try {
    ...
    const results = await deliveryDriverService.syncOfflineCompletions(req.user, items);
